...
Leia o tutorial introdutório ao uso do GRC, disponível na página \url{http://wiki.gnuradio.org/index.php/Guided_Tutorial_GRC}, a partir da seção 2.2. 

Se possível, instale o GRC em seu computador pessoal (veja instruções de instalação em \url{http://wiki.gnuradio.org/index.php/InstallingGR}) e experimente os exercícios sugeridos no tutorial.
...
\begin{questions}
     \question Determine as expressões para os coeficientes ($a_n, b_n$) das séries trigonométricas de Fourier para as formas de onda periódicas ilustradas na Figura \ref{fig:sinaisusuais} e compare os valores dos coeficientes com os valores calculados numericamente no \textit{Jupyter notebook}. Estão de acordo?
   ...
   % \question Determine as séries de Fourier trigonométricas das formas de onda periódicas ilustradas na Figura \ref{fig:sinaisusuais}.
    \begin{figure}[htb]
        \centering
        \subfigure[fig:ondaquadrada][Onda quadrada $g_{q}(t)$ de frequência $f = 100$~Hz e amplitude $A = 1$.]{\includegraphics{./Figuras/OndaQuadrada}}\label{subfig:ondaquadrada}
        \qquad
        \subfigure[fig:ondatriangular][Onda triangular $g_{t}(t)$ de frequência $f = 100$~Hz e amplitude $A = 1$.]{\includegraphics{./Figuras/OndaTriangular}}
        \qquad
        \subfigure[fig:ondadenteserra][Onda dente de serra $g_{d}(t)$ de frequência $f = 100$~Hz e amplitude $A = 1$.]{\includegraphics{./Figuras/OndaDenteSerra}}
...
A seguir são descritas práticas experimentais a serem realizadas pelo aluno. 
...
\begin{enumerate}
    ...
    \item Ajuste os valores dos parâmetros \textbf{atenuacao} e \textbf{atraso} nos blocos \textbf{Variable}, à esquerda do diagrama, com valores de atenuação e atraso (em número de amostras) de canal desejados, começando com os valores 1 e 0, respectivamente. 
    \item Execute o diagrama e responda às questões propostas na Folha de Respostas.
\end{enumerate}
...
\begin{questions}
    \question Observe que os sinais de entrada e saída do canal são idênticos. Esse seria um canal ideal, que sequer atenua ou atrasa o sinal transmitido.
    \begin{parts}
        \part Ajuste a variável {\bf atenuacao} para 0.5 e execute o experimento. Houve distorção? Explique o que ocorreu.
        \fillwithlines{0.5in}
        \part Ajuste a variável {\bf atraso} para 4, que corresponde a 0,5~ms (por quê?), e execute o experimento. Houve distorção? Explique o que ocorreu.
        \fillwithlines{0.5in}
    \end{parts}

    \question Retorne os valores iniciais das variáveis \textbf{atenuacao} e \textbf{atraso} (1 e 0 respectivamente). Observe nos parâmetros dos blocos \textbf{Multiply Const} e \textbf{Delay} que seus valores são sempre iguais, definidos pelas variáveis \textbf{atenuacao} e \textbf{atraso}. Ajuste apenas o bloco {\bf Multiply Const} de uma das componentes do sinal para \textbf{atenuacao/2.0} e execute o experimento. Houve distorção? Explique o que ocorreu.
    \fillwithlines{0.5in}
    
    \question Retorne os valores iniciais das variáveis \textbf{atenuacao} e \textbf{atraso} (1 e 0 respectivamente). Ajuste apenas o bloco \textbf{Delay} de uma das componentes do sinal para \textbf{atraso + 4} e execute o experimento. Houve distorção? Explique o que ocorreu.
    \fillwithlines{0.5in}
    
    \question Retorne os valores iniciais das variáveis \textbf{atenuacao} e \textbf{atraso} (1 e 0 respectivamente). Troque em todos os blocos \textbf{Signal Source} o parâmetro \textbf{Waveform}: de Cosine para Sine e execute o experimento. Houve distorção? Explique o que ocorreu.
    \fillwithlines{1.0in}
\end{questions}

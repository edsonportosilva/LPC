...
$h(n)=\begin{cases}
\frac{\Omega_{c_2}-\Omega_{c_1}}{\pi}, n=\alpha, \\
\frac{sin(\Omega_{c_1} \left(n-\alpha\right))-sin(\Omega_{c_2} \left(n-\alpha\right))}{\pi \left(n-\alpha\right)} , n\neq \alpha
\end{cases}$ \\
...
Perceba que uma simulação computacional, como a do Gnuradio, utiliza sinais discretos no tempo. Isto é, a variável temporal não é contínua e os sinais são representados à uma dada frequência de amostragem (sampling rate) $f_s$. Como consequência do teorema de Nyquist, a frequência de amostragem $f_s=\omega_s/2\pi$ impõe um intervalo limite $[-f_s/2,f_s/2]$ para a largura de banda máxima dos sinais discretos. Dessa forma, sinais e sistemas numa simulação computacional naturalmente são limitados em banda. Desse modo, ao gerar-se em simulação um processo aleatório AWGN de variância $\sigma^2$ à uma taxa de amostragem $f_s$, a potência total média do ruído simulado será de $P_N = \sigma^2f_s$ W.

\subsection{Problemas}

\begin{questions}

\question Projete um filtro passa-baixas de ordem $N = 25$. Determine os coeficientes $h(i)$, com a resposta em frequência dada na Tabela \ref{tab:filtrosReais}, para $\omega_s = 2\pi 8000$~rad/s e $\omega_c = 2\pi 1000$~rad/s. Por fim, esboce um gráfico $h(n)$ versus $n$ para os valores calculados. A curva deve ter forma de uma função $\operatorname{sinc}{(\cdot)}$ truncada e deslocada da origem.
